...
use crate::db::backend::{NoopStorageBackend, StorageBackend};
use crate::db::persistence::{PersistenceDriver, PersistentMetric};
...
/// have one. Receives the session's persistence driver so persistent metrics
/// can register their row codecs alongside their buffers.
type BufferRegistration = fn(
    &mut BufferStore,
    &mut PersistenceDriver,
    i64,
    &HashSet<MetricDependency>,
) -> Result<(), String>;
...
impl MetricRegistration {
    pub fn ephemeral<T: Metric>() -> Self {
        Self {
            ...
            register: |store, _driver, buffer_size_ms, demand| {
...
impl MetricEngine {
    ...
    pub fn live_session(&self, config: LiveSessionConfig) -> Result<LiveSession, String> {
        self.live_session_with_storage(config, Box::new(NoopStorageBackend))
    }

    /// Live session that flushes buffered metrics to the provided persistent
    /// storage backend at the configured interval.
    pub fn live_session_with_storage(
        &self,
        config: LiveSessionConfig,
        backend: Box<dyn StorageBackend>,
    ) -> Result<LiveSession, String> {
        LiveSession::new(config, self, backend)
    }
